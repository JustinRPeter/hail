...
#Only do this if lightning.thresold = zero
...
#ex_vars <- c("MUCAPE_SMG","LR75_SMG","H5_TEMP_SMG","S06_SMG","PMR_SMG","HAIL")
ex_vars <- c("LR75_SMG","H5_TEMP_SMG","S06_SMG","PMR_SMG","HAIL")
dfex <- data[,ex_vars]
...
train <- sample_frac(dfex,train.fraction)
test <- subset(dfex,!rownames(dfex) %in% rownames(train))



#Perform random forest classification
#rf <- randomForest(as.factor(HAIL) ~ MUCAPE_SMG+LR75_SMG+
#                                     H5_TEMP_SMG+S06_SMG+PMR_SMG,
rf <- randomForest(as.factor(HAIL) ~ LR75_SMG+H5_TEMP_SMG+S06_SMG+PMR_SMG,
                   #data=dfex,ntree=100,mtry=3,importance=TRUE,do.trace=1)
                   data=dfex,ntree=100,importance=TRUE,do.trace=1)
...
cat("Predicting probability of hail using randomForest model: ", "\n")
pred_test <- predict(rf,newdata=era_data,type="vote",norm.votes=TRUE) #All years

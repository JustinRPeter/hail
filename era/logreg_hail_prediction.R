...
##Perform a logistic regression on all the hail data
...
#model.null <- glm(HAIL ~ 1, 
#                 data=train,
#                 family = binomial(link="logit"))
#
#model.full <- glm(HAIL~LR75_SMG+H5_TEMP_SMG+S06_SMG+PMR_SMG,
#                  data=train,
#                  family=binomial(link="logit"))
#
##This will find the significant paramters in the model
##I think thaat all of them are significant
#step(model.null,scope = list(upper=model.full),
#     direction="both",
#     test="Chisq",
#     data=train)
#
#hail.model <- model.full
#
##Now construct ROC curves to test model
#fitted.model <- predict(hail.model,
#                           newdata=subset(test,
#                           select=c(LR75_SMG,H5_TEMP_SMG,S06_SMG,PMR_SMG)),
#                           type="response")
#pr <- prediction(fitted.model, test$HAIL)
#prf <- performance(pr, measure = "tpr", x.measure = "fpr")
#plot(prf)
#auc <- performance(pr, measure = "auc")
#auc <- auc@y.values[[1]]
#auc
#fitted.model <- ifelse(fitted.model > 0.5,1,0)
#misClasificError <- mean(fitted.model != test$HAIL)
#

#Now try the regression on the scaled data
...
train <- sample_frac(dfn,train.fraction)
test <- subset(dfn,!rownames(dfn) %in% rownames(train))
...
#model.full <- glm(HAIL~MUCAPE_SMG+LR75_SMG+H5_TEMP_SMG+S06_SMG+PMR_SMG,
model.full <- glm(HAIL~LR75_SMG+H5_TEMP_SMG+S06_SMG+PMR_SMG,
                  data=train,
                  family=binomial(link="logit"))
...
fitted.model <- predict(hail.model,
                           ...
                           #select=c(LR75_SMG,H5_TEMP_SMG,S06_SMG,PMR_SMG)),
                           #select=logreg_vars),
                           select=tail(names(step.model$coefficients),-1),
                           type="response"))
pr <- prediction(fitted.model, test$HAIL)
prf <- performance(pr, measure = "tpr", x.measure = "fpr")
plot(prf)
...
source("colScale.R")
cat("Scaling ERA data before logistic regression hail probability prediction: ","\n")
...
cat("Predicting probability of hail using logistic regression model: ", "\n")
pred_test <- predict(hail.model,newdata=era_data_n,type="response") #All years
...
hail_clim <- apply(pred_test,c(1,2),sum,na.rm=TRUE)/(7*4)

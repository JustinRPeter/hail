#This is very similar to era_gpass_hail_analysis.R
#but here we extract each of the SHIP components
#at the maximum lightning count during a day.
...
source("/home/jpeter/justinp/rfiles/lib/thermo/thermo.r")
...
#Write to a file
sink("hail_predictor.txt",type='output',append=FALSE)
#Put the variable names at the header of the file
cat("Lon","Lat","Date_ERA","Time_ERA","Date_GPATS","Time_GPATS",
    "Max_SHIP","MUCAPE_MS","LR75_MS","H5_TEMP_MS","S06_MS","PMR_MS",
    "SHIP_Max_GPATS","MUCAPE_SMG","LR75_SMG","H5_TEMP_SMG","S06_SMG","PMR_SMG",
    "Max_GPATS","Hail_Occurred", "\n")
...
#for (ill in 10){ 
  ...
  for (i in seq_along(scomps)){
      ex.scomps[[i]] <- ex.scomps[[i]][range.dates_era]
  }
  ...
  #This doesn't mean we exclude no hail days
  #It is because the computation will bomb if length(hail_dates)=0!
  #It is because not all of the identified suitable grid boxes 
  #necessarily recorded a hail event.
  if (length(hail_dates > 0)){
    ...
    scomps.at.max.lightning.hail <- vector("list",length=length(scomps))
    scomps.at.max.lightning.no.hail <- vector("list",length=length(scomps))

    chunk.hail.scomps <- vector("list",length=length(scomps))
    chunk.no.hail.scomps <- vector("list",length=length(scomps))
    #max.hail.scomps.time.ind <- vector("list",length=length(scomps))
    #max.no.hail.scomps.time.ind <- vector("list",length=length(scomps))
    max.hail.scomps <- vector("list",length=length(scomps))
    max.no.hail.scomps <- vector("list",length=length(scomps))
...
#---#(1) Evaluate the parameters for all the HAIL days
    ...
    for (i in 1:len.chunks.hail.era){
        ...
        for (j in seq_along(scomps)){
            chunk.hail.scomps[[j]] <- ex.scomps[[j]][chunk.hail.ship.time.inds]
        }
        ...
        #for (j in seq_along(scomps)){
        #    max.hail.scomps.time.ind[[j]][i] <- which.max(chunk.hail.scomps[[j]])
        #}
        #max.hail.ship[i] <- max(chunk.hail.ship)
        #Evaluate the SHIP at this indice
        max.hail.ship[i] <- chunk.hail.ship[max.hail.ship.time.ind[i]]
        for(j in seq_along(scomps)){
            max.hail.scomps[[j]][i] <- chunk.hail.scomps[[j]][max.hail.ship.time.ind[i]]
        }
    ...
    #   #Find the closest ERA time to the max. lightning time
        ...
        for (j in seq_along(scomps)){
            scomps.at.max.lightning.hail[[j]][i] <-
                                         chunk.hail.scomps[[j]][closest.time.ind.hail]
        }
    }
...
#---#(2) Evaluate the parameters for all the NO HAIL days
    ...
    for (j in 1:len.chunks.no.hail.era){
        ...
        for (k in seq_along(scomps)){
            chunk.no.hail.scomps[[k]] <-
                ex.scomps[[k]][chunk.no.hail.ship.time.inds]
        }
        #Evaluate the indice of the max. ship
        max.no.hail.ship.time.ind[j] <- which.max(chunk.no.hail.ship)
        #for (k in seq_along(scomps)){
        #    max.no.hail.scomps.time.ind[[k]][j] <-
        #                                which.max(chunk.no.hail.scomps[[k]])
        #}
        #Evaluate the SHIP at this indice
        max.no.hail.ship[j] <- chunk.no.hail.ship[max.no.hail.ship.time.ind[j]]
        for (k in seq_along(scomps)){
            max.no.hail.scomps[[k]][j] <-
                        chunk.no.hail.scomps[[k]][max.no.hail.ship.time.ind[j]]
        }
        #Evaluate the time of the maximum SHIP
    ...
    for (j in 1:len.chunks.no.hail.gpats){
        ...
        chunk.no.hail.ship <- ex.ship[chunk.no.hail.ship.time.inds]
        for (k in seq_along(scomps)){
            chunk.no.hail.scomps[[k]] <- ex.scomps[[k]][chunk.no.hail.ship.time.inds]
        }
    #   #Find the closest ERA time to the max. lightning time
        ...
        for (k in seq_along(scomps)){
            scomps.at.max.lightning.no.hail[[k]][j] <-
                                    chunk.no.hail.scomps[[k]][closest.time.ind.no.hail]
        }
    }
    ...
    for (i in 1:len.chunks.hail.era){
        cat(ex.lon,ex.lat,
            ...
            max.hail.ship[i],
            sapply(max.hail.scomps,'[[',i),
            ship.at.max.lightning.hail[i],
            sapply(scomps.at.max.lightning.hail,'[[',i),
    ...
    for (j in 1:len.chunks.no.hail.era){
        cat(ex.lon,ex.lat,
            ...
            max.no.hail.ship[j],
            sapply(max.no.hail.scomps,'[[',j),
            ship.at.max.lightning.no.hail[j],
            sapply(scomps.at.max.lightning.no.hail,'[[',j),
